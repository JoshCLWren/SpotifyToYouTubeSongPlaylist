...
logging.basicConfig(
    filename=f"{arrow.now().format('YYYY-MM-DD')}-errors.log", level=logging.DEBUG
)
...
async def handle_request(func, time_to_wait=1, time_waited=0, **kwargs):
    ...
    try:
        ...
        call = func(**kwargs)
        logging.warning(
            f"{arrow.now().isoformat()}{time_waited} seconds waited, no error, for {func.__name__}"
        )
        return call
        ...
        if e.status_code not in [*range(200, 300)]:
            ...
            return handle_request(
                func, time_waited=time_waited, time_to_wait=time_to_wait, **kwargs
            )

        print(e)
        print(
            f"Error in {func.__repr__}, waiting {time_waited} seconds, kwargs: {kwargs}"
        )
